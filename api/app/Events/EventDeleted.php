...
/**
 * Broadcast event fired when a daily event is deleted from a location.
 *
 * Trigger:  Dispatched from EventController::destroy() after the Event
 *           record is removed from the database.
 * Channel:  Private "location.{id}" -- scoped to the deleted event's location.
 * Payload:  A plain array containing at least `id` and `location_id` (captured
 *           before deletion) so the client knows which event to remove.
 * Purpose:  Notifies all connected clients at the location to remove the
 *           event from their UI in real time.
 *
 ...
 *           deleted by the time the event is dispatched.
 */
class EventDeleted implements ShouldBroadcast
{
     ...
     *                       `location_id` keys, representing the event that
     *                       was just removed.
     */
     ...
     * can identify and remove the deleted event from the list.
     */
